...
export default function FallingBehindPage() {
    ...
    const loop = (time: number) => {
      ...
      if (orbReady.current && orbImage.current && orbs.current.length) {
        ...
        orbs.current.forEach((orb, idx) => {
          ...
          const size = orb.r * 2;
          ctx.globalAlpha = 0.9;
          const imgCurrent = orbImage.current;
          if (imgCurrent && imgCurrent.width && imgCurrent.height) {
            const aspect = imgCurrent.width / imgCurrent.height;
            const drawW = size;
            const drawH = size / aspect;
            ctx.drawImage(imgCurrent, -drawW / 2, -drawH / 2, drawW, drawH);
          }
          ctx.restore();
        });
      }
  ...
  return (
    <main className="relative min-h-[100svh] overflow-hidden bg-neutral-950 text-neutral-50">
      <nav className="relative z-20 mx-auto flex max-w-4xl items-center gap-2 px-6 pt-6 text-[11px] uppercase tracking-[0.3em] text-white/50">
        ...
        <span className="opacity-60">/</span>
        <span className="text-white">Falling Behind</span>
      </nav>
      <canvas ref={canvasRef} className="absolute inset-0" />

      <FallingBehindText showCTAs={phase >= 3} />
    </main>
  );
}
...
function FallingBehindText({ showCTAs = false }: { showCTAs?: boolean }) {
  return (
    <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-4xl flex-col items-center justify-center px-6 py-16">
      <div className="relative w-full max-w-[40ch] text-center">
        ...
        <div className="relative">
          ...
          >
            Falling behind doesn’t have to mean staying behind.
            <br />
            Backlog anxiety gets replaced with a controlled recovery plan.
          </motion.p>
          ...
          >
            The system surfaces the backlog honestly.
            <br />
          ...
          >
            Catch-up paths are generated automatically.
            <br />
            Progress resumes without overwhelm.
          </motion.p>
        </div>
        ...
        >
          ...
          <br />
          Stress drops as momentum returns.
        </motion.p>
        ...
        {showCTAs && (
          ...
          >
            <Link
              href="/problems/falling-behind/handle-it"
