...
import type { HandleItContent } from "@/data/problemHandleIt";
import { SOLUTION_LOOKUP } from "@/data/problemHandleIt";

export default function HandleItExperience({ problem, content }: { problem: Problem; content: HandleItContent }) {
  const solutionSections = content.solutionSections.map((section) => ({
    ...section,
    solutions: section.solutionSlugs.map((slug) => SOLUTION_LOOKUP[slug]).filter(Boolean),
  }));
  ...
  return (
    <main className="bg-neutral-950 text-neutral-50">
      <nav className="mx-auto flex max-w-6xl items-center gap-2 px-6 pt-6 text-[11px] uppercase tracking-[0.3em] text-white/50">
        ...
        <span className="opacity-60">/</span>
        <Link href={`/problems/${problem.slug}`} className="transition hover:text-white">
          {problem.label}
        </Link>
        <span className="opacity-60">/</span>
      ...
      <section className="relative overflow-hidden border-b border-white/5 bg-gradient-to-b from-[#0b1a3a] via-[#0f2352] to-[#091327]">
        ...
        <div className="relative mx-auto flex max-w-6xl flex-col gap-10 px-6 py-20 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-6 lg:max-w-3xl">
            <Link href={`/problems/${problem.slug}`} className="inline-flex items-center text-sm text-white/70 transition hover:text-white">
            ...
            {section.solutions.length > 0 && (
              <div className="mt-10 grid gap-6 md:grid-cols-2">
                {section.solutions.map((solution) => (
                  ...
                  >
                    <div>
                      <p className="text-xs uppercase tracking-[0.35em] text-white/60">{solution.type === "owned" ? "owned" : "curated"}</p>
      ...
      <section className="border-t border-white/10 bg-neutral-950">
        <div className="mx-auto flex max-w-5xl flex-col items-center gap-6 px-6 py-20 text-center">
          ...
          <div className="flex w-full max-w-md flex-col gap-4">
            ...
            >
              Replay the {problem.label.toLowerCase()} canvas →
            </Link>
            <Link
              href="/problems"
